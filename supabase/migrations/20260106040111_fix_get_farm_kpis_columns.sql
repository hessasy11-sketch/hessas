/*
  # تصحيح دالة get_farm_kpis لاستخدام الأعمدة الصحيحة
  
  تعديل الدالة لاستخدام:
  - approved_at بدلاً من completed_at
  - الحالات الصحيحة من جدول farm_tasks
*/

-- =====================================================
-- دالة: get_farm_kpis (محدثة)
-- =====================================================
...
DECLARE
  ...
  v_result json;
BEGIN
  -- 1. عدد المهام المكتملة (آخر 30 يوم) - المعتمدة
  ...
  -- 2. عدد المهام المتأخرة (معلقة أو قيد التنفيذ وتجاوزت due_date)
  ...
  -- 3. متوسط وقت إغلاق المهمة (بالساعات) - من الإنشاء إلى الموافقة
  ...
  -- 5. عدد الاعتمادات المرفوضة (مهام + مصروفات - آخر 90 يوم)
...
-- =====================================================
-- دالة: get_farm_performance_trend (محدثة)
-- =====================================================
...
BEGIN
  ...
  WHERE farm_id = p_farm_id
    AND status = 'approved'
    AND approved_at >= CURRENT_DATE - (p_days || ' days')::interval;
  ...
  WHERE farm_id = p_farm_id
    AND status = 'approved'
    AND approved_at >= CURRENT_DATE - (p_days * 2 || ' days')::interval
    AND approved_at < CURRENT_DATE - (p_days || ' days')::interval;
...
-- Comments
-- =====================================================
COMMENT ON FUNCTION get_farm_kpis IS 'مؤشرات أداء شاملة للمزرعة (محدثة) - يستخدم approved_at للمهام المعتمدة';
COMMENT ON FUNCTION get_farm_performance_trend IS 'اتجاه الأداء للمزرعة (محدث) - مقارنة بين فترتين';
