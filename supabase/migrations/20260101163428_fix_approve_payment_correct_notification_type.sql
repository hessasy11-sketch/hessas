/*
  # إصلاح دالة approve_payment_document - استخدام notification_type الصحيح
  
  المشكلة الحقيقية:
  - الدالة تستخدم notification_type = 'payment_approved'
  - لكن الـ constraint يسمح فقط بـ: 'payment', 'booking', 'contract', إلخ
  
  الحل: استخدام 'payment' بدلاً من 'payment_approved'
*/
...
BEGIN
  ...
  -- إرسال إشعار للمستثمر (استخدام 'payment' بدلاً من 'payment_approved')
...
-- دالة لرفض المستند المالي (مع تصحيح notification_type)
...
BEGIN
  ...
  -- إرسال إشعار للمستثمر (استخدام 'payment' بدلاً من 'payment_rejected')
  INSERT INTO b2f_notifications (
    investor_phone,
    title,
    message,
