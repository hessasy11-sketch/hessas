/*
  # إنشاء stored procedures لإصدار العقود
  
  1. الدوال الجديدة
    - `issue_contract_for_request`: إصدار عقد لطلب واحد
    - `issue_multiple_contracts`: إصدار عقود متعددة دفعة واحدة
    
  2. الآلية
    - التحقق من أن الطلب ready_for_contract = true
    - إنشاء سجل في b2f_contracts
    - توليد رقم عقد فريد
    - تحديث الطلب: status = 'contract_issued', contract_id = العقد الجديد
    - إنشاء أمر تشغيل في b2f_operations_orders (إذا كان الجدول موجوداً)
*/
...
BEGIN
  ...
  INSERT INTO b2f_contracts (
    ...
    'investment',
    NOW(),
    NOW() + (COALESCE(request_record.duration_years, 5) || ' years')::INTERVAL,
    'active',
    'pending_start'
  )
  ...
  EXCEPTION WHEN undefined_table THEN
    -- لا نفعل شيء إذا لم يكن الجدول موجوداً
    NULL;
  END;
