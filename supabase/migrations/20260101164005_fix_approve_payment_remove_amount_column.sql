/*
  # إصلاح دالة approve_payment_document - إزالة عمود amount
  
  المشكلة:
  - الدالة تحاول إدراج في عمود 'amount' غير موجود
  - جدول b2f_financial_operations_log لا يحتوي على عمود amount
  
  الحل: إزالة عمود amount من عملية الإدراج
*/
...
BEGIN
  ...
  -- تسجيل في سجل العمليات المالية (بدون عمود amount)
...
-- دالة لرفض المستند المالي (مع نفس الإصلاح)
