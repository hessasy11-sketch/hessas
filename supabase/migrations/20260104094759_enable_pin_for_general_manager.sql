/*
  # تفعيل الرقم السري للمدير العام

  1. التعديلات
    - تفعيل requires_pin للمدير العام (general_manager)
    - إضافة رمز PIN = 1234 للمدير العام
    - التأكد من التوجيه إلى /hq بعد التحقق

  2. الأمان
    - استخدام crypt لتشفير الرمز السري
*/

-- تحديث المدير العام لتفعيل PIN وتعيين الرمز السري
...
-- تحديث verify_qr_access لجعل general_manager يتطلب PIN
...
BEGIN
  ...
  -- تحديد إذا كان يتطلب PIN
  -- المدير العام يحتاج PIN، باقي الأدوار لا
  v_requires_pin := CASE 
    WHEN v_staff_record.role = 'general_manager' THEN true
    ELSE false
  END;
